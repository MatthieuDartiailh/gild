...
from enaml.widgets.api import Dialog, PopupView, Window
...
def handle_dialog(
    qtbot,
    op="accept",
    handler=lambda qtbot, window: window,
    cls=Dialog,
    time=100,
    skip_answer=False,
):
...
def handle_question(qtbot, answer):
    ...
    with handle_dialog(
        qtbot,
        "accept" if answer == "yes" else "reject",
...
def set_preferences(workbench, preferences):
    ...
    """
    plugin = workbench.get_plugin("glaze.preferences")
    plugin._prefs = OrderedDict(preferences)
...
def signal_error_raise():
    ...
    from glaze.plugins.errors.plugin import ErrorsPlugin
...
def exit_on_err(self):
    """Replacement function for glaze.plugins.errors plugin exit_error_gathering.
    ...
    def list_referrers(self, exclude=[], depth=0):
        ...
        def find_referrers(ref_dict, obj, depth=0, exclude=[]):
            ...
            referrers = [
                ...
                if not (inspect.isframe(ref) or ref in exclude)
            ]
            if depth == 0:
                ...
                deeper = {}
                for r in referrers:
                    find_referrers(deeper, r, depth - 1, exclude + [obj, referrers])
                ref_dict[obj] = deeper
