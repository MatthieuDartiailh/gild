...
"""
from atom.api import Bool, Callable, Dict, Int, List, Value, set_default
...
class QtListStrWidget(RawWidget):
    ...
    def on_selection(self):
        ...
        if not self._guard & INDEX_GUARD:
            ...
            widget = self.get_widget()
            selected = [self._rmap[index.row()] for index in widget.selectedIndexes()]
            if selected:
                if self.multiselect:
    ...
    def _post_setattr_multiselect(self, old, new):
        ...
        widget.setSelectionMode(mode)
        if self.items:
            self._select_on_widget(self.selected_items if new else [self.selected_item])
    ...
    def _select_on_widget(self, items, widget=None):
        ...
        if widget is None:
            widget = self.get_widget()
        if widget is not None:
            widget.setCurrentItem(widget.item(0), QtCore.QItemSelectionModel.Clear)
            item_map = self._map
            for n in items:
                widget.setCurrentItem(
                    widget.item(item_map[n]), QtCore.QItemSelectionModel.Select
                )
