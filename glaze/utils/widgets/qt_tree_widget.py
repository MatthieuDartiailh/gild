...
from atom.api import Bool, Dict, Int, List, Value
...
class QtTreeWidget(RawWidget):
    ...
    def _node_for(self, obj):
        ...
        if (type(obj) is tuple) and (len(obj) == 2) and isinstance(obj[1], TreeNode):
            return obj
    ...
    def _on_context_menu(self, pos):
        ...
        context = {
            "copyable": self._is_copyable(obj, node, parent_node),
        ...
        if menu is not None:
            if not all(
                (not action.visible or action.separator) for action in menu.items()
            ):
    ...
    def _is_cutable(self, obj, node, parent_node):
        can_cut = (
            ...
            and parent_node.can_copy(obj)
            and parent_node.can_delete(obj)
        )
    ...
    def _is_deletable(self, obj, node, parent_node):
        can_delete = (parent_node is not None) and parent_node.can_delete(obj)
        return bool(can_delete and node.can_delete_me(obj))

    def _is_renameable(self, obj, node, parent_node):
        can_rename = (parent_node is not None) and parent_node.can_rename(obj)
            ...
            for expanded, node, nid in self._object_info_for(obj, name):
                ...
                if expanded:
                    ...
                    for index, child in change.added:
                        child, child_node = self._node_for(child)
                        if child_node is not None:
                            self._insert_node(nid, index, child_node, child)
                        else:
...
class _TreeWidget(QtWidgets.QTreeWidget):
    ...
    def dropEvent(self, e):
        ...
        if action == "move" and dragging is not None:
            ...
            if dragging is not None:
                data = control._drop_object(to_node, to_object, data, False)
                if data is not None:
                    from_node, from_object, from_index = control._node_index(dragging)
                    if (to_object is from_object) and (to_index > from_index):
                        to_index -= 1
                    control._delete(from_node, from_object, from_index)
                    control._insert(to_node, to_object, to_index, data, False)
            else:
                data = control._drop_object(to_node, to_object, data, True)
                if data is not None:
                    control._insert(to_node, to_object, to_index, data, False)
    ...
    def _get_action(self, event):
        ...
        # node.
        if event.proposedAction() == QtCore.Qt.MoveAction and control._is_droppable(
            node, obj, data, False
        ):
