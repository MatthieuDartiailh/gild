...
#: Global variable linked to the --glaze-sleep cmd line option.
DIALOG_SLEEP = 0
...
def pytest_configure(config):
    """Turn the --glaze-sleep command line into a global variable."""
    s = config.getoption("--glaze-sleep")
    if s is not None:
        global DIALOG_SLEEP
        DIALOG_SLEEP = s * 1000
...
@pytest.fixture
def dialog_sleep():
    """Return the time to sleep as set by the --glaze-sleep option."""
    return DIALOG_SLEEP
